...
export const authOptions: NextAuthOptions = {
  ...
  providers: [
    TwitchProvider({
      ...
      authorization: { params: { scope: 'user:read:email chat:read chat:edit', force_verify: true } },
    }),
  ],
  callbacks: {
    async jwt({ token, account, profile }) {
      if (account) {
        ...
        const expiresAtSec =
          ...
          (typeof (account as any).expires_in === 'number' ? nowSec + Number((account as any).expires_in) : undefined)
